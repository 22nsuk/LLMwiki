...
| `ops/scripts/release/external_report_lifecycle_runtime.py` | `build_report()` assembles inventory, profile, lifecycle, and release-verification evidence through sub-runtime imports plus in-module reason-target tables. | Move inventory/profile loading and report-text ingestion into dedicated loader helpers while keeping sub-runtime imports as the first seam. | Move action-status resolver tables, lifecycle assembly, and archive-reconciliation reason mapping into decision helpers. | Move lifecycle report envelope assembly and summary rendering into renderer helpers. | Keep release-verified action promotion rules in `external_report_release_verification_runtime.py` and related verification helpers. | Next safe move: extract inventory/profile loader glue before touching reason-target tables; delete unused compat export tuples. |
| `ops/scripts/release/release_closeout_batch_manifest.py` | `build_report()` / `main()` coordinate ZIP inventory, digest checks, release decision assembly, and envelope rendering in one module. | Extract artifact inventory and upstream report loading into a batch-manifest loader helper. | Extract digest mismatch, status classification, and release decision dataclasses into decision helpers. | Extract envelope/report assembly into a renderer helper and keep CLI in a thin façade. | Keep promotion/finality blocking rules with existing closeout authority helpers. | Next safe move: extract ZIP timestamp normalization and artifact inventory loading first. |
| `ops/scripts/mechanism/goal_runtime_run_admission.py` | `build_report()` plus CLI entrypoint coordinate 12+ upstream report loads and gate decisions. | Share JSON loading through `goal_runtime_json_loader_runtime.py` and move upstream report loading into an admission loader helper. | Extract gate classification and runnable-queue recomputation into decision helpers independent of readiness snapshots. | Extract admission report envelope assembly into a renderer helper. | Keep promotion/admission blocking rules aligned with goal-runtime certificate semantics. | Next safe move: adopt shared goal-runtime JSON loader across admission/closeout/preflight modules before splitting gate decisions. |
...
| `ops/scripts/mechanism/goal_runtime_run_admission.py` and goal-runtime cluster | `goal_runtime_json_loader_runtime.py` | Loader: shared vault/path JSON loading for admission, closeout, preflight, stale closeout, fixed-point, and clean-transient modules. | Cross-cutting hygiene before admission loader extraction. | Goal runtime JSON loader focused tests plus existing goal-runtime admission/closeout tests. |
