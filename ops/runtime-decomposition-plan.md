...
| `ops/scripts/mechanism/auto_improve_readiness_runtime.py` readiness cluster | `build_readiness_report()` loads reports, derives queue state, assesses execution/learning/promotion readiness, and renders the readiness report. | Done for queue derivation ownership: `ops/scripts/mechanism/auto_improve_readiness_queue_runtime.py` owns `ReadinessQueueState`, and the façade now carries that object instead of duplicating queue scalar fields. | Done for `_dict_field` leakage: readiness queue/release-authority/worktree-guard helpers now share `ops/scripts/core/payload_field_runtime.py::dict_field()` instead of importing private helpers across ownership boundaries. Move remaining queue check/remediation/fallback decisions behind queue-state-oriented helpers without changing the readiness JSON contract. | Keep readiness report assembly in the façade until queue/check/fallback payload helpers are byte-pinned and source paths stay stable. | Keep admission independent: `goal_runtime_run_admission.py` must recompute runnable queue/currentness instead of trusting readiness' snapshot. | Next safe move: move queue payload/check/remediation assembly to queue-owned helpers under focused readiness tests. |
...
| `ops/scripts/mechanism/auto_improve_readiness_runtime.py` | Existing `ReadinessQueueState` from `ops/scripts/mechanism/auto_improve_readiness_queue_runtime.py` is now carried by `ReadinessInputs`. | Loader/decision ownership: queue derivation stays in the queue runtime while the façade no longer unpacks and re-owns queue scalar fields. | Readiness runtime, readiness queue, queue selection, goal admission, next-run repair, and schema sample tests plus static checks. |
