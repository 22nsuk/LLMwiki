...
def _normalized_artifact_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in ENVELOPE_REQUIRED_FIELDS:
        if field not in embedded_envelope:
            continue
        if _envelope_field_missing(field, normalized.get(field)):
            normalized[field] = embedded_envelope[field]
    return normalized
...
def _schema_validation(vault: Path, payload: dict[str, Any]) -> tuple[str, list[str]]:
...
def _artifact_record_schema_state(
    ...
    *,
    ...
    if payload and not noncanonical_archive:
        schema_validation_status, schema_validation_errors = _schema_validation(vault, payload)
        if schema_validation_status == "fail":
...
def _collect_artifact_freshness_scan_inputs(
    ...
    *,
    ...
    artifact_records = [
        _json_artifact_record(vault, path, mtime_source=mtime_source, zip_mtimes=zip_mtimes)
