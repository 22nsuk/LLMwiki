...
| Public mirror/export | Corpus-free code/ops runtime for public review, tests, and CI. | `docs/`, `ops/`, `tests/`, `tools/`, `mk/`, `.agents/skills/`, `.codex/agents/`, `.github/`, and root public documents/config. | `raw/`, `wiki/`, `system/`, `runs/`, `external-reports/`, `ops/operator/`, `ops/reports/`, `tmp/`, and private inventory files. | `ops/scripts/public/public_surface_policy.py`. | `make sync-derived` and `make public-export`. | `make sync-derived-check`, `make public-check`, or `make public-check-all`. |
