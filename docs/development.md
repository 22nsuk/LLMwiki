...
| Auto-promotion evidence | `make release-auto-promotion-ready-check` | Run with `GOAL_RUN_ID=<promoted-run-id>` or let `make release-auto-promotion-goal-run-id-guard` infer it from matching current `goal-run-status` and `goal-runtime-certificate` evidence; `make release-auto-promotion-preflight`, `make release-run-ready`, `make release-auto-promotion-preseal`, `make release-sealed-run-ready`, then `make release-auto-promotion-ready` when unattended promotion is the intended outcome; preflight/preseal inject the guard's effective run id into run-local evidence refresh, preseal includes `make release-auto-promotion-safe-cleanup`, and Stage 3 reuses the sealed operator summary generated by the sealed stage |
