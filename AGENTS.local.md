...
- 아래 규칙은 `raw/`, `wiki/`, `system/`, `runs/`를 직접 다룰 때만 추가로 적용한다.
...
- `ops/`, `tests/`, `tools/`, `.codex/agents/`, `.github/` 중심의 public-safe runtime maintenance
...
3. markdown raw면 corpus mutation에 들어가기 전에 먼저 검토하고, contract가 허용하는 범위에서 metadata/noise를 정리한다.
   - 기본 경로는 `raw_markdown_normalize --report-only`로 점검하고, 필요할 때만 `--write`로 반영한다.
...
해야 할 일:
1. `ops/evals/`의 binary eval suite를 읽는다.
2. `ops/scripts/wiki_lint.py`와 `ops/scripts/wiki_eval.py` 결과를 본다.
3. 실패 패턴 하나를 고른다.
...
2. `promotion_gate.py`로 `runs/<run-id>/promotion-report.json` 생성
...
## 10. Scaling Rule And First Priorities
...
초기 우선순위:
1. `system/lint--initial-review-2026-04-12.md` 기준 bootstrap 결함 정리
2. source / concept / synthesis canonical page 지속 확충
3. `ops/scripts/wiki_lint.py`와 `wiki_eval.py`를 CI 또는 pre-session check에 연결
4. major planning work에는 seed freeze + planning gate 도입
5. meta-loop를 `autoresearch` 스타일 keep/discard 실험으로 자동화
