...
def fixed_context() -> RuntimeContext:
    return RuntimeContext(
        ...
        clock=lambda: dt.datetime(2026, 5, 24, 8, 0, tzinfo=dt.timezone.utc),
    )
...
class LintUpliftPlanTests(unittest.TestCase):
    ...
    def test_report_uses_full_scope_preview_targets_without_blocking_static(self) -> None:
        ...
        self.assertFalse(report["strict_preview"]["uses_legacy_allowlist"])
    ...
    def test_legacy_allowlist_mode_is_visible(self) -> None:
        (self.vault / "mk" / "static.mk").write_text(
            "ruff-strict-preview:\n"
            "\tpython tools/ruff_strict_preview.py --allowlist ops/ruff-strict-preview-allowlist.txt\n",
            encoding="utf-8",
        )

        report = build_report(self.vault, targets=["ops/scripts"], context=fixed_context())

        self.assertEqual(report["strict_preview"]["target_mode"], "legacy_target_list")
        self.assertTrue(report["strict_preview"]["uses_legacy_allowlist"])
