...
class ExportPublicRepoTests(unittest.TestCase):
    ...
    def test_export_public_repo_copies_only_public_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (vault / "THIRD_PARTY_NOTICES.md").write_text(
                "# Third-Party Notices\n", encoding="utf-8"
            )
            (vault / "docs").mkdir()
            (vault / "docs" / "README.md").write_text("# Docs\n", encoding="utf-8")
            (vault / "docs" / "development.md").write_text(
                "# Development\n", encoding="utf-8"
            )
            ...
            (vault / ".github" / "CODEOWNERS").write_text(
                "* @example\n", encoding="utf-8"
            )
            ...
            (vault / "tools").mkdir()
            (vault / "tools" / "helper.py").write_text(
                "print('helper')\n", encoding="utf-8"
            )
            ...
            (vault / "system").mkdir()
            (vault / "system" / "system-index.md").write_text(
                "# system\n", encoding="utf-8"
            )
            (vault / "runs").mkdir()
            (vault / "runs" / "README.md").write_text("# runs\n", encoding="utf-8")
            (vault / "external-reports").mkdir()
            (vault / "external-reports" / "report.pdf").write_text(
                "pdf", encoding="utf-8"
            )
            (vault / "ops" / "raw-registry.json").write_text("{}", encoding="utf-8")
            (vault / "ops" / "reports").mkdir()
            (vault / "ops" / "reports" / "artifact.json").write_text(
                "{}", encoding="utf-8"
            )
            (vault / "ops" / "reports" / "goal-worktree-guard.json").write_text(
                "{}", encoding="utf-8"
            )
            (
                vault / "ops" / "reports" / "release-workflow-order-guard.json"
            ).write_text("{}", encoding="utf-8")
            (vault / "ops" / "reports" / "workflow-dependency-planner.json").write_text(
                "{}", encoding="utf-8"
            )
            ...
            self.assertNotIn(
                "ops/operator/operator-release-summary.json", manifest["files"]
            )
            ...
            self.assertNotIn(
                "ops/reports/release-workflow-order-guard.json", manifest["files"]
            )
            self.assertNotIn(
                "ops/reports/workflow-dependency-planner.json", manifest["files"]
            )
