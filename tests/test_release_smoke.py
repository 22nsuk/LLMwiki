...
class ReleaseSmokeTest(unittest.TestCase):
    ...
    def test_build_release_archive_uses_manifest_inventory_and_excludes_local_only_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            (vault / "wiki").mkdir(exist_ok=True)
            (vault / "wiki" / "private.md").write_text("wiki private", encoding="utf-8")
            (vault / "system").mkdir(exist_ok=True)
            (vault / "system" / "system-log.md").write_text("system private", encoding="utf-8")
            ...
            self.assertFalse(any(entry["path"].startswith("system/") for entry in source_manifest["files"]))
            ...
            self.assertIn("AGENTS.local.md", source_manifest["exclusion_policy"]["excluded_files"])
            self.assertIn("raw/", source_manifest["exclusion_policy"]["excluded_prefixes"])
