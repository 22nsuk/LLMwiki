...
def build_ruff_command(select: str, targets: list[str]) -> list[str]:
    if not targets:
        raise ValueError("strict Ruff allowlist must contain at least one target")
    return [sys.executable, "-m", "ruff", "check", "--select", select, *targets]


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run strict Ruff preview on an allowlisted surface")
